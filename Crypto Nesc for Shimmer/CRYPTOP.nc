...
static unsigned char rc[10] = {0x63, 0x7C, 0x77, 0x7B, 0xF2, 0x6B, 0x6F, 0xC5, 0x30, 0x01};
...
/////////////////////////////////////AES///////////////////////////////////////////////////
void key_addition(unsigned char data [][4],unsigned char k [][4], unsigned char A [][4]){
    int i,j;
    for (i = 0; i < 4; ++i)
        for (j = 0; j < 4; ++j)
            A[j][i] =  (data[j][i]) ^ (k[j][i]);
}

void byte_substituition(unsigned char A[][4], unsigned char B[][4]){
    unsigned char Lnibble;
    unsigned char Mnibble;
    int i,j;
    for (i = 0; i < 4; ++i)
        for (j = 0; j < 4; ++j) {
            Lnibble = (unsigned char) ((A[j][i]) & (0x0f));
            Mnibble = (unsigned char) (((A[j][i]) & (0xf0)) >> 4);
            B[j][i] = S_Box[Mnibble][Lnibble];
        }
}

void shift_rows(unsigned char B[][4], unsigned char SRB[][4]){
    int i,j;
    for (i = 0; i < 4; ++i)
        for (j = 0; j < 4; ++j) {
            SRB[i][j] = B[i][(j + i) % 4];
...
unsigned char galoi_mult_2(unsigned char B){
    unsigned int temp = 0;
    ...
    if (B%0x80 >> 7){
        ...
        temp = temp <<1;
        temp = temp ^ 0x011b;
        result = (unsigned char)(temp & 0x00ff);
    }
    else{
        temp = (unsigned int )(B);
        temp = temp <<1;
...
unsigned char galoi_mult_3(unsigned char B){
    unsigned int temp = 0;
    unsigned char result = 0;
    if (B%0x80>>7){
        temp = (unsigned int )(B);
...
void mix_column(unsigned char SRB[][4],unsigned char C[][4]){
    int i;
    for (i = 0; i < 4; ++i) {
        C[0][i] = (galoi_mult_2(SRB[0][i])) ^ (galoi_mult_3(SRB[1][i])) ^ SRB[2][i] ^ SRB[3][i];
        C[1][i] = SRB[0][i] ^ (galoi_mult_2(SRB[1][i]) ^ (galoi_mult_3(SRB[2][i])) ^ SRB[3][i]);
        C[2][i] = SRB[0][i] ^ SRB[1][i] ^ (galoi_mult_2(SRB[2][i]) ^ galoi_mult_3(SRB[3][i]));
        C[3][i] = (galoi_mult_3(SRB[0][i])) ^ SRB[1][i] ^ SRB[2][i] ^ galoi_mult_2(SRB[3][i]);
    }
}

void gfunc(const unsigned char* v, unsigned char i, unsigned char* b){
    unsigned char Lnibble, Mnibble, s;
    int j;
    b[0] = v[1];
    b[1] = v[2];
    b[2] = v[3];
    ...
    Lnibble = (unsigned char) (b[0]& 0x0f);
    Mnibble = (unsigned char) ((b[0]& 0xf0)>>4);
    s = S_Box[Mnibble][Lnibble];
    b[0] = s ^ rc[i-1];
    for (j = 0; j < 4; ++j) {
        Lnibble = (unsigned char)(b[j] & 0x0f);
        Mnibble = (unsigned char)((b[j] & 0xf0) >> 4);
        b[j] = S_Box[Mnibble][Lnibble];
    }
}

async command CRYPTO.subkey(unsigned char k [][4], unsigned char w [][4]){
    unsigned char g [4], e[4];
    int i,j,r,l,x,q,z;
    for (i = 0; i < 4; ++i)
        for (j = 0; j < 4; ++j)
            w[i][j] = k[j][i];
    for (r = 1; r <= 10; ++r) {
        ...
        gfunc(g, (unsigned char)(r), e);
        for (x = 0; x < 4; ++x)
            w[4*r][x] = w[4*(r-1)][x] ^ e [x];
        for (q = 1; q <= 3 ; ++q)
            for (z = 0; z < 4; ++z)
                w[4*r+q][z] = w[4*r+q-1][z] ^ w[4*(r-1) + q][z];
    }
}

async command CRYPTO.aes_cipher(unsigned char in[][4], unsigned char out[][4], unsigned char w[][4]) {
    int i,j,r;
    for (i = 0; i < 4; i++)
        for (j = 0; j < 4; j++)
            state[i][j] = in[i][j];

    key_addition(state, w, A);

    for (r = 0; r < Nr - 1; r++) {
        byte_substituition(A, B);
        shift_rows(B,SRB);
        mix_column(SRB,state);
        key_addition(state, w, A);
    }
    byte_substituition(A, B);
    shift_rows(B,SRB);
    key_addition(SRB, w, state);
